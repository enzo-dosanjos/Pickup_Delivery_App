...
class XMLParsersTest {
    ...
    void checkParseRequestsLoadsWarehouseRequests() {
        ...
        assertEquals(208769039, pickupDelivery.getRequests().get(pickupDelivery.getRequestsPerCourier().get(1L).get(0)).getPickupIntersectionId(), "The first request address for courier 1 should match the expected value");
        assertEquals(25173820, pickupDelivery.getRequests().get(pickupDelivery.getRequestsPerCourier().get(1L).get(0)).getDeliveryIntersectionId(), "The first request delivery address for courier 1 should match the expected value");
        assertEquals(180, pickupDelivery.getRequests().get(pickupDelivery.getRequestsPerCourier().get(1L).get(0)).getPickupDuration().toSeconds(), "The first request pickup duration for courier 1 should match the expected value");
        assertEquals(240, pickupDelivery.getRequests().get(pickupDelivery.getRequestsPerCourier().get(1L).get(0)).getDeliveryDuration().toSeconds(), "The first request delivery duration for courier 1 should match the expected value");
    }
